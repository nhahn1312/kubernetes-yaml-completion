...
export class ResolvedSchema {
    ...
    public getSection(path: string[]): JSONSchema | undefined {
        ...
        if (schemaRef) {
            return ValidationUtil.asSchema(schemaRef);
        }
        return undefined;
    }
...
export class JSONSchemaService implements IJSONSchemaService {
    ...
    private cachedSchemaForResource:
        | {
              resource: string;
              resolvedSchema: Thenable<ResolvedSchema | undefined>;
          }
        | undefined;
        ...
        return this.requestService(url).then(
            ...
            (error: any) => {
                ...
                return new UnresolvedSchema(<JSONSchema>{}, [
                    l10n.t("Unable to load schema from '{0}': {1}.", toDisplayString(url), errorMessage)
                ]);
            }
        );
    }
    ...
    public resolveSchemaContent(schemaToResolve: UnresolvedSchema, handle: SchemaHandle): Thenable<ResolvedSchema> {
        ...
        const schemaDraft = schema.$schema ? normalizeId(schema.$schema) : undefined;
        if (schemaDraft === 'http://json-schema.org/draft-03/schema') {
            return this.promise.resolve(
                new ResolvedSchema({}, [l10n.t('Draft-03 schemas are not supported.')], [], schemaDraft)
            );
        }
        ...
        const mergeRef = (
            ...
            if (section) {
                ...
                resolveErrors.push(
                    l10n.t("$ref '{0}' in '{1}' can not be resolved.", refSegment || '', sourceHandle.uri)
                );
            }
        };
        ...
        const resolveExternalLink = (
            ...
            return referencedHandle.getUnresolvedSchema().then((unresolvedSchema) => {
                ...
                if (unresolvedSchema.errors.length) {
                    ...
                    resolveErrors.push(
                        l10n.t("Problems loading reference '{0}': {1}", loc, unresolvedSchema.errors[0])
                    );
                }
    ...
    public getSchemaForResource(resource: string, document?: JSONDocument): Thenable<ResolvedSchema | undefined> {
        if (document) {
